...
public class ContactsSynchronizer {
    ...
    public ContactsSynchronizer(String firebaseUrl, String appId) {
        ...
        Utils.d(DEBUG_CONTACTS_SYNC, "contactsNode : " + contactsNode.toString());
    }

    public synchronized ChildEventListener connect() {
        Utils.d(DEBUG_CONTACTS_SYNC, "connecting  for contacts ");

//        contactsNode.orderByChild("firstname").addListenerForSingleValueEvent(new ValueEventListener() {
//            public void onDataChange(DataSnapshot dataSnapshot) {
//               Utils.d("TAG", dataSnapshot.toString());
//               Utils.d("TAG", "We're done loading the initial " + dataSnapshot.getChildrenCount() + " items");
//            }
//
//            @Override
...
//        });
        ...
        if (contactsChildEventListener == null) {
            ...
            Utils.d(DEBUG_CONTACTS_SYNC, "creating a new contactsChildEventListener");
                    ...
                    new AsyncTask<DataSnapshot, Void, Void>() {
                        ...
                        protected Void doInBackground(final DataSnapshot... snapshots) {
                            new Thread(new Runnable() {
                                public void run() {
                                    Utils.v(DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildAdded");

                                    try {
                                        IChatUser contact = decodeContactSnapShop(snapshots[0]);
                                        Utils.d(DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildAdded.contact : " + contact);
                                        if (!ChatManager.getInstance().getLoggedUser().getId().equals(contact.getId()))
                                        {
                                            addContact(contact);
                                        }else {
                                            IChatUser currentUser = ChatManager.getInstance().getLoggedUser();
                                            if (currentUser.getFullName() == null || currentUser.getFullName().length() < 1 )
                                            {
                                                currentUser.setFullName(contact.getFullName());
                                            }

                                            currentUser.setSetTenantName(contact.getTenantName());
                                        }
                                    } catch (ChatFieldNotFoundException cfnfe) {
                                        Utils.w(DEBUG_CONTACTS_SYNC, "Error decoding contact on onChildAdded " + cfnfe.getMessage());
...
//                            Utils.v(DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildChanged");

                    try {
                        IChatUser contact = decodeContactSnapShop(dataSnapshot);
                        updateLoggedUser(contact);
                        Utils.d(DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildChanged.contact : " + contact);
                        updateContact(contact);
                    } catch (ChatFieldNotFoundException cfnfe) {
                        Utils.w(DEBUG_CONTACTS_SYNC, "Error decoding contact on onChildChanged " + cfnfe.getMessage());
...
//                    }.execute(dataSnapshot);
                ...
                public void onChildRemoved(DataSnapshot dataSnapshot) {
                    Utils.v(DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildRemoved");

                    try {
                        IChatUser contact = decodeContactSnapShop(dataSnapshot);

                        Utils.d(DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildRemoved.contact : " + contact);
                        ...
                        Utils.w(DEBUG_CONTACTS_SYNC, "Error decoding contact on onContactRemoved " + cfnfe.getMessage());
                    } catch (Exception e) {
                        if (contactListeners != null) {
                            for (ContactListener contactListener : contactListeners) {
...
//                Utils.d(TAG, "observeMessages.onCancelled");
            ...
            Utils.i(DEBUG_CONTACTS_SYNC, "connected for contacts ");

        } else {
            Utils.i(DEBUG_CONTACTS_SYNC, "already connected to contacts ");
        }
    ...
    private void saveOrUpdateContactInMemory(IChatUser contact) {
        Utils.d(DEBUG_CONTACTS_SYNC, "saveOrUpdateContactInMemory  for contact : " + contact);

        int index = contacts.indexOf(contact);

        if (index > -1) {
            contacts.set(index, contact);
            Utils.v(DEBUG_CONTACTS_SYNC, "contact " + contact +
                    "updated into contacts at position " + index);

        } else {
            contacts.add(contact);
            Utils.v(DEBUG_CONTACTS_SYNC, "contact " + contact + "is not found into contacts." +
                    " The contact was added at the end of the list");
        }
    }

//    private void saveOrUpdateContactOnStorage(IChatUser contact) {
//        Utils.d(DEBUG_CONTACTS_SYNC, "saveOrUpdateContactOnStorage  for contact : " + contact);
//
...
//            Utils.d(DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.saveOrUpdateContactOnStorage:" +
...
//    }
    ...
    public void upsertContactsListener(ContactListener contactListener) {
        Utils.v(DEBUG_CONTACTS_SYNC, "  upsertContactsListener called");
        ...
        if (contactListeners.contains(contactListener)) {
            ...
            Utils.i(DEBUG_CONTACTS_SYNC, "  contactListener with hashCode: " + contactListener.hashCode() + " updated");

        } else {
            this.addContactsListener(contactListener);
            Utils.i(DEBUG_CONTACTS_SYNC, "  contactListener with hashCode: " + contactListener.hashCode() + " added");
        }
    }

    public void addContactsListener(ContactListener contactListener) {
        Utils.v(DEBUG_CONTACTS_SYNC, "  addContactsListener called");
    ...
    public void removeContactsListener(ContactListener contactListener) {
        Utils.v(DEBUG_CONTACTS_SYNC, "  removeContactsListener called");

        this.contactListeners.remove(contactListener);

        Utils.i(DEBUG_CONTACTS_SYNC, "  contactListener with hashCode: " + contactListener.hashCode() + " removed");
    }

    public void removeAllContactsListeners() {
        this.contactListeners.clear();
        Utils.i(DEBUG_CONTACTS_SYNC, "Removed all contactListeners");
    }
...
//        }
    ...
    public static IChatUser decodeContactSnapShop(DataSnapshot dataSnapshot) throws ChatFieldNotFoundException {
        Utils.v(DEBUG_CONTACTS_SYNC, "decodeContactSnapShop called");
        ...
        contact.setSetTenantName(tenantName);
        Utils.v(DEBUG_CONTACTS_SYNC, "decodeContactSnapShop.contact : " + contact);
