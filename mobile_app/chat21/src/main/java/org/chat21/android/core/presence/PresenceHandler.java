...
public class PresenceHandler {
    ...
    public void upsertPresenceListener(PresenceListener presenceListener) {
        if (presenceListeners.contains(presenceListener)) {
            ...
            Utils.i(DEBUG_USER_PRESENCE, "PresenceHandler.upsertPresenceListener: " +
    ...
    public ValueEventListener connect() {
        ...
        valueEventListener = userPresenceRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                Utils.i(DEBUG_USER_PRESENCE, "PresenceHandler.connect.onDataChange:" +
                        " dataSnapshot == " + dataSnapshot);

                long lastOnline = LAST_ONLINE_UNDEFINED;
                long lastActivity = LAST_ONLINE_UNDEFINED;
                ...
                if (dataSnapshot.getValue() != null) {
                    ...
                    try {
                        ...
                        Utils.w(DEBUG_USER_PRESENCE, "PresenceHandler.connect.onDataChange:" +
                                " Cannot retrieve lastOnline " + e.toString());
                    }

                    try {
                        lastActivity = (long) value.get("lastActivity");
                        if (lastActivity>0)
                            lastActivity = new Date(lastActivity).getTime();

                    } catch (Exception e) {
                        Utils.w(DEBUG_USER_PRESENCE, "PresenceHandler.connect.onDataChange:" +
                                " Cannot retrieve lastActivity " + e.toString());
                    }
                    ...
                    if (connections != null && connections.size() > 0) {
                        online = true;
                        if (lastActivity>0 && System.currentTimeMillis() > (lastActivity + (2*60*1000)))
                        {
                            online = false;
                        }
                    }
            ...
            public void onCancelled(DatabaseError databaseError) {
                Utils.w(DEBUG_USER_PRESENCE, "PresenceHandler.connect.onCancelled: " +
