...
class RecipientViewHolder extends RecyclerView.ViewHolder {
    ...
    private void setMessage(Message message) {
        // set message text
        if (message.getTranslatedText() !=null && message.getTranslatedText().trim().length()>0 && Utils.isTranslationEnabled(itemView.getContext()))
            mMessage.setText(Html.fromHtml(message.getTranslatedText()));
        else {
