...
public class MarkerView extends View {
    ...
    private void intit()
    {
        ...
        Bitmap bt= Util.getBitmapFromAsset(mContext.getAssets(),"markerbg.jpg");
        mWidth=bt.getWidth();
    ...
    private void loadSymbols()
    {
        ...
        for (int i = 0; i < checkoutMarksInfo.size(); i++) {
            ...
            if (mrk.getType() == Constant.SYMBOL_MARKER_SCRATCH_THIN || mrk.getType() == Constant.SYMBOL_MARKER_SCRATCH) {
                LineMarker line = new LineMarker(boundingRect.left + (int) (mrk.getX1() * widthScaleFactor), boundingRect.top + (int) (mrk.getY1() * heightScaleFactor), boundingRect.left + (int) (mrk.getX2() * widthScaleFactor), boundingRect.top + (int) (mrk.getY2() * heightScaleFactor), mrk.getType(), Color.BLUE, heightScaleFactor);
                line.setIsEditable(mrk.isDeleteable());
                imageMarkers.add(line);
                line.setId(mrk.getId());

            } else if (mrk.getType() == Constant.SYMBOL_MARKER_LARGE_DENT || mrk.getType() == Constant.SYMBOL_MARKER_SMALL_DENT) {
                boolean islarge = false;
                if (mrk.getType() == Constant.SYMBOL_MARKER_LARGE_DENT)
                    islarge = true;
                CircleMarker dent = new CircleMarker(boundingRect.left + (int) (mrk.getX1() * widthScaleFactor), boundingRect.top + (int) (mrk.getY1() * heightScaleFactor), mrk.getType(), Color.BLUE, heightScaleFactor, islarge);
                dent.setIsEditable(mrk.isDeleteable());
            ...
            else if(mrk.getType()==Constant.SYMBOL_MARKER_CRACK)
            {
                ...
                crack.setIsEditable(mrk.isDeleteable());
    ...
    public void addmark(int x,int y,int x2,int y2,int type,String id,boolean isDeletionEnabled)
    {

        PreviousMarkerIno mrk=new PreviousMarkerIno(x,y,x2,y2,type,id,isDeletionEnabled);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int ww=(int)(h*imageAspectRation);    // scaled width w.r.t height
        ...
        //Util.LogE("Image res", mWidth + " : " + mHeight + " " + imageAspectRation);
        //Util.LogE("View Res", ww + " : " + h);
                ...
                case MotionEvent.ACTION_DOWN:
                    ...
                    if(symbolMarker!=null)
                    {
                       clearSelection();
                        ...
                        {
                            clearSelection();
                            LineMarker line=new LineMarker(lastX,lastY,lastX,lastY,currentSymbolId,ContextCompat.getColor(mContext, R.color.colorPrimary),heightScaleFactor);
